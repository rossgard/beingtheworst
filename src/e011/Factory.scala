...
class FactoryAggregate(val aggregateState: FactoryState) {
  ...
  def assignEmployeeToFactory(employeeName: String) {
    ...
    if (aggregateState.listOfEmployeeNames.contains(employeeName)) {
      fail(":> FAIL: the name of '%s' only one employee can have\n", employeeName)
      return
    }
    if (employeeName == "bender") {
      fail(":> FAIL: Guys with the name 'bender' are trouble.")
      return
    }
  ...
  def transferShipmentToCargoBay(shipmentName: String, parts: Array[CarPart]) {
    ...
    if (aggregateState.listOfEmployeeNames.isEmpty) {
    ...
    if (aggregateState.shipmentsWaitingToBeUnloaded.size > 2) {
  ...
  def receiveShipmentInCargoBay(shipmentName : String, parts : Array[CarPart]) {

  }

  def fail(message: String, args: String*) {
...
class ShipmentReceivedInCargoBay(shipmentName : String, parts : Array[CarPart]) extends  Event {
  override def toString = shipmentName + " with parts " + parts mkString ", " + " arrived at factory"
}
